(function () {
  ...
  for (i = 0; i < l; i++) {
    ...
    slideList.push({
      ...
      hasInnerNavigation: null !== slides[i].querySelector('.inner') ||
            window.innerNavigation[slides[i].id] !== undefined,
      navigationCallback: window.innerNavigation[slides[i].id]
    });
  }
  ...
  function increaseInnerNavigation(slideNumber) {
    ...
    if (slideList[slideNumber].navigationCallback)
        return slideList[slideNumber].navigationCallback();
